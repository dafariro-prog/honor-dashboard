 ...
 * de Honor (multi-país) y cruzándolas con data/homologacion.json
 * (platform||account_name -> {hom:'Honor', pais:<corregido>, ag:'Honor'}).
 *
 * Importante: en la homologación original todas las cuentas Honor estaban mal
 * etiquetadas como país = Panamá; aquí el país viene ya corregido en homologacion.json.
 *
...
(async () => {
  ...
  const out = {
    ...
    cliente: 'Honor',
    nota: 'Datos reales de BigQuery (cuentas Honor, todo 2026) · país corregido desde el account_name · gasto en moneda nativa sin convertir',
  ...
  console.log(`OK · ${detalle.length} campañas · ${rows.length} filas de serie · ` +
    `${new Set(detalle.map(d => d.pais)).size} países · ${new Set(detalle.map(d => d.plataforma)).size} plataformas`);
